    ...
    return (
      <div className="min-h-screen flex items-center justify-center wood-bg">
        <div className="text-center px-6">
          ...
          <h1 className="font-playfair text-3xl font-bold text-gold-400 mb-3">المنتج غير موجود</h1>
            ...
            <div className="flex flex-col">
                ...
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {p.sz.map((s, i) => (
                      <button
                        key={s.s}
                        className={`px-4 py-2.5 rounded-lg border text-gold-300 text-sm transition-all hover:bg-gold-500/10 ${i === 0 ? "border-gold-500 ring-2 ring-gold-500 bg-gold-500/10" : "border-gold-500/20"}`}
                      >
              ...
              <div className="md:hidden sticky bottom-0 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 pt-4 pb-2" style={{ background: "linear-gradient(to top, #1A0F0A 60%, transparent)" }}>
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    <p className="font-playfair text-gold-300 font-bold text-lg">{p.sz[0].p.toLocaleString()} <span className="text-gold-400/50 text-xs font-normal">ج.م</span></p>
                    <p className="text-gold-100/30 text-xs">{p.sz[0].s}</p>
                  </div>
                  <button
                    onClick={() => addToCart({ id: p.id, name: p.ar, size: p.sz[0].s, price: p.sz[0].p, img: p.img })}
              ...
              <div className="hidden md:block">
                <div className="flex items-center gap-4 p-4 rounded-xl gold-border" style={{ background: "rgba(45,27,17,.3)" }}>
                  <div className="flex-1">
                    <span className="font-playfair text-2xl font-bold text-gold-300">{p.sz[0].p.toLocaleString()}</span>
                    <span className="text-gold-400/50 text-sm mr-1">ج.م</span>
                    <p className="text-gold-100/30 text-xs mt-0.5">{p.sz[0].s} - أسعار تبدأ من {p.sz[p.sz.length - 1].p.toLocaleString()} ج.م</p>
                  </div>
                  <button
                    onClick={() => addToCart({ id: p.id, name: p.ar, size: p.sz[0].s, price: p.sz[0].p, img: p.img })}
              ...
              {sameFamily.map((s) => (
                ...
                >
                  ...
                  <div className="p-3">
                    ...
                    <p className="text-gold-400 text-xs font-bold mt-1">{s.sz[0].p.toLocaleString()} ج</p>
                  </div>
                </div>
              ))}
            </div>
